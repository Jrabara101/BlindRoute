...
import { BlindRoute, type BlindRoutePrivateState, witnesses } from '@midnight-ntwrk/blindroute-contract';
...
import {
  type BlindRouteCircuits,
  type BlindRouteContract,
  BlindRoutePrivateStateId,
  type BlindRouteProviders,
  type DeployedBlindRouteContract,
} from './common-types';
 ...
 * by evaluating the pure `commitmentOf` circuit against a throwaway local context.
 * No network call, no ledger state — this is the same hash both the customer
 * (when locking) and courier (when releasing) can compute independently.
 */
export const commitmentOf = (proof: Uint8Array): Uint8Array => {
  const localContract = new BlindRoute.Contract<BlindRoutePrivateState>(witnesses);
  const { currentContractState, currentZswapLocalState, currentPrivateState } = localContract.initialState(
    createConstructorContext(generateBlindRoutePrivateState(), '0'.repeat(64)),
  );
  const context = {
    currentPrivateState,
  ...
  return localContract.circuits.commitmentOf(context, proof).result;
};

export const getEscrowLedgerState = async (
  providers: BlindRouteProviders,
  ...
  const state = await providers.publicDataProvider.queryContractState(contractAddress).then((contractState) => {
    ...
    return {
      state: ['EMPTY', 'LOCKED', 'RELEASED'][escrow.state] as EscrowLedgerState['state'],
...
export const blindRouteContractInstance: BlindRouteContract = new BlindRoute.Contract(witnesses);

export const joinContract = async (
  providers: BlindRouteProviders,
  contractAddress: string,
  privateState: BlindRoutePrivateState,
): Promise<DeployedBlindRouteContract> => {
  const blindRouteContract = await findDeployedContract(providers, {
    contractAddress,
    compiledContract: blindRouteCompiledContract,
    privateStateId: BlindRoutePrivateStateId,
    initialPrivateState: privateState,
  });
  logger.info(`Joined contract at address: ${blindRouteContract.deployTxData.public.contractAddress}`);
  return blindRouteContract;
};
...
export const deploy = async (
  ...
  logger.info('Deploying BlindRoute escrow contract...');
  const blindRouteContract = await deployContract(providers, {
    compiledContract: blindRouteCompiledContract,
    privateStateId: BlindRoutePrivateStateId,
    initialPrivateState: privateState,
  });
  logger.info(`Deployed contract at address: ${blindRouteContract.deployTxData.public.contractAddress}`);
  return blindRouteContract;
};
...
export const displayEscrowState = async (
  providers: BlindRouteProviders,
  blindRouteContract: DeployedBlindRouteContract,
): Promise<{ escrow: EscrowLedgerState | null; contractAddress: string }> => {
  const contractAddress = blindRouteContract.deployTxData.public.contractAddress;
  const escrow = await getEscrowLedgerState(providers, contractAddress);
  if (escrow === null) {
    logger.info(`There is no BlindRoute contract deployed at ${contractAddress}.`);
  } else {
    logger.info(`Escrow state: ${escrow.state}, amount: ${escrow.amount}`);
  }
...
${DIV}
  ...
  https://faucet.preprod.midnight.network/  (or /preview/ for Preview network)
${DIV}
`);
...
export const configureProviders = async (ctx: WalletContext, config: Config) => {
  ...
  const zkConfigProvider = new NodeZkConfigProvider<BlindRouteCircuits>(contractConfig.zkConfigPath);
  ...
  return {
    privateStateProvider: levelPrivateStateProvider<typeof BlindRoutePrivateStateId>({
